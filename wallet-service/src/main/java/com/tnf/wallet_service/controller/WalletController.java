...
public class WalletController {
    ...
    public ResponseEntity<WalletDTO> getWalletById(@PathVariable String walletId) {
        ...
        return ResponseEntity.ok(toDto(walletService.getWalletById(walletId)));
    }

    @GetMapping("/customer/{customerId}")
    public ResponseEntity<List<WalletDTO>> getWalletsByCustomer(@PathVariable String customerId) {
    ...
    public ResponseEntity<WalletDTO> addMoney(@PathVariable String walletId,
            @Valid @RequestBody AddMoneyRequest request) {
        logger.info("POST /api/wallets/{}/add-money - amount {}", walletId, request.getAmount());
        return ResponseEntity.ok(toDto(walletService.addMoney(walletId, request.getAmount())));
    }

    @PostMapping("/{walletId}/pay-bill")
    public ResponseEntity<WalletDTO> payBill(@PathVariable String walletId,
            @Valid @RequestBody PayBillRequest request) {
        logger.info("POST /api/wallets/{}/pay-bill - amount {}", walletId, request.getAmount());
        return ResponseEntity.ok(toDto(walletService.payBill(walletId, request.getAmount())));
    }

    @PostMapping("/{walletId}/transfer")
    public ResponseEntity<WalletDTO> transfer(@PathVariable String walletId,
