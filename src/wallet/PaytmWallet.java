...
public class PaytmWallet implements WalletOperations {
    ...
    public void addMoney(double amount)
        ...
        balance += amount;
        System.out.println("  [Paytm] ₹" + amount + " added. Wallet balance: ₹" + balance);
    }
    ...
    public void payBill(String billType, double amount)
        ...
        if (amount > balance) {
            throw new InsufficientBalanceException(
                    "InsufficientBalanceException: Insufficient wallet balance to pay " +
                            billType + " bill. Available: ₹" + balance + ", Required: ₹" + amount);
        }
        balance -= amount;
        System.out.println("  [Paytm] " + billType + " bill of ₹" + amount +
    ...
    public void transferToWallet(WalletOperations targetWallet, double amount)
        ...
        if (amount > balance) {
            throw new InsufficientBalanceException(
                    "InsufficientBalanceException: Insufficient Paytm wallet balance. " +
                            "Available: ₹" + balance + ", Requested: ₹" + amount);
        }
        balance -= amount;
        dailyTransferredAmount += amount;
        targetWallet.addMoney(amount);
        System.out.println("  [Paytm] Transferred ₹" + amount + " to target wallet.");
    }
    ...
    public void displayBalance() {
        System.out.println("  [Paytm Wallet] ID: " + walletId +
                " | Owner: " + ownerName +
                " | Balance: ₹" + balance);
    }
}
